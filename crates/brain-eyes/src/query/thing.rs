...
fn extras(
    ...
    match kind {
        ...
        "source_file" => {
            extras.briefing = briefing_rows(loaded, sid, slug, now)?;
        }
        "agent_session" => {
...
pub(crate) fn briefing_rows(
    ...
    rel: &str,
    now: u64,
) -> Result<Vec<Concern>, String> {
    use brain_observe::briefing::{self, WriteAccess};
    let store = &loaded.store;
    let index = &loaded.index;
    let prefix = loaded.prefix();
    let name = format!("{prefix}/{rel}");
    let b = briefing::before(store, index, prefix, &name, sid).map_err(|e| e.to_string())?;
    let mut rows = Vec::new();
    let mut push = |severity: &str, title: String, reason: String, fix: Option<String>| {
        rows.push(Concern {
    ...
    match &b.write {
        WriteAccess::Projection {
            ...
            "act",
            "do not edit this file".to_string(),
            format!(
                "it is rendered from the {} “{slug}” — the graph owns it",
                say::kind_noun(kind)
            ),
            Some(edit_via.clone()),
        ),
        WriteAccess::Captured { kind } => push(
            "note",
            "editable".to_string(),
            format!(
                "recorded as a {} when it changes",
                say::kind_noun(kind)
            ),
            None,
        ),
        WriteAccess::File => push(
            "note",
            "editable".to_string(),
            "the graph observes changes when it next looks".to_string(),
            None,
        ),
    }

    if b.blast.transitive == 0 {
        push(
            "note",
            "nothing imports this file".to_string(),
            "an edit here stays here".to_string(),
            None,
        );
    } else {
        ...
        let mut reason = format!("directly: {}", shown.join(", "));
        if more > 0 {
            reason.push_str(&format!(" and {more} more"));
        }
        push(
            if b.blast.transitive >= 10 { "watch" } else { "note" },
            format!(
                "an edit here reaches {}",
                say::count(b.blast.transitive as u64, "other file", "other files")
            ),
            reason,
            None,
        );
    ...
    if b.covered_by.is_empty() {
        push(
            "watch",
            "no test covers this file".to_string(),
            "changes here land unproven".to_string(),
            None,
        );
    } else {
        push(
            "note",
            format!("covered by {}", b.covered_by.join(", ")),
            "a change here shows up in the next run".to_string(),
            None,
        );
    }

    for note in b.notes.iter().take(3) {
        let (severity, title, text) = match brain_observe::twin::note_kind(&note.text) {
            Some(("dead-end", body)) => ("watch", "a dead end was recorded here", body),
            Some((kind, body)) => ("note", match kind {
                "learning" => "a session learned something here",
                "gap" => "a gap was recorded here",
                _ => "a decision is pending here",
            }, body),
            None => ("note", "a session left a note here", note.text.as_str()),
        };
        let mut clipped: String = text.chars().take(200).collect();
        if text.chars().count() > 200 {
            clipped.push('\u{2026}');
        }
        push(
            severity,
            format!("{title}, {}", say::ago(now, note.at_ms)),
            clipped,
            None,
        );
    }
